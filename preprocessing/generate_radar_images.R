...
vertical_integration <- function(datetime){
    ...
    h5g = H5Gopen(fid, "where")
    h5writeAttribute.character(attr = as.character(r_attr$crs), h5obj = h5g, name = "projdef")
    H5Gclose(h5g)

    h5g = H5Gopen(fid, "how")
    h5writeAttribute.double(attr = lon_min, h5obj = h5g, name = "lon_min")
    h5writeAttribute.double(attr = lat_min, h5obj = h5g, name = "lat_min")
    h5writeAttribute.double(attr = lon_max, h5obj = h5g, name = "lon_max")
    h5writeAttribute.double(attr = lat_max, h5obj = h5g, name = "lat_max")
    h5writeAttribute.array(attr = res(r), h5obj = h5g, name = "resolution")
    h5writeAttribute.integer(attr = r_attr$ncols, h5obj = h5g, name = "xsize")
    h5writeAttribute.integer(attr = r_attr$nrows, h5obj = h5g, name = "ysize")

    h5g = H5Gopen(fid, "what")
    h5writeAttribute.character(attr = config$quantity, h5obj = h5g, name = "quantity")
    h5writeAttribute.character(attr = "IMAGE", h5obj = h5g, name = "object")
    h5writeAttribute.array(attr = config$radars, h5obj = h5g, name = "source")
    h5writeAttribute.character(attr = datetime, h5obj = h5g, name = "datetime")
    H5Fclose(fid)

...
def training(cfg: DictConfig, output_dir: str, log):
    ...
    for epoch in range(cfg.model.epochs):
        ...
        model.train()
        flux_loss_weight = cfg.model.get('flux_loss_weight', 0)
        for nidx, data in enumerate(train_loader):
            data = data.to(device)
            optimizer.zero_grad()
            if hasattr(model, 'teacher_forcing'):
                model.teacher_forcing = tf
            output = model(data)
            gt = data.y
            ...
            if n_iter % 100 == 0:
                ...
                if cfg.verbose:
                    print(f'iteration {n_iter}: loss = {training_curve[-1]}', file=log)
                    print(f'iteration {n_iter}: val loss = {val_loss}', file=log)
                    log.flush()
                ...
                if cfg.model.early_stopping and n_iter % cfg.model.avg_window == 0:
                    # every X epochs, check for convergence of validation loss
                    if n_iter == 0:
                        l = val_curve[0]
                    else:
                        l = np.mean(val_curve[(n_iter - (cfg.model.avg_window - 1)):])
...
def train(model, train_loader, optimizer, loss_func, device, teacher_forcing=0, **kwargs):
    ...
    for nidx, data in enumerate(train_loader):
        ...
        loss = loss_func(output, gt, mask) + penalty
        loss_all.append(data.num_graphs * float(loss))
        loss.backward()
        optimizer.step()

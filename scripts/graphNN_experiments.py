...
def run_training(timesteps, model_type, conservation=True, recurrent=True, embedding=0, norm=False, epochs=100,
    ...
    for r in range(repeats):
        ...
        for epoch in range(epochs):
            loss = train_fluxes(model, train_loader, optimizer, boundaries, loss_func, 'cpu',
...
def plot_test_errors(timesteps, model_names, short_names, model_types, output_path,
    ...
    for midx, model in enumerate(models):
        ...
        if short_names[midx] == 'standard_mlp':
            loss_all[short_names[midx]].append(
                test_fluxes(model, test_loader, timesteps, loss_func, 'cpu',
                     get_outfluxes=False, bird_scale=bird_scale)
            )
        else:
            l, outfluxes, outfluxes_abs = test_fluxes(model, test_loader, timesteps, loss_func, 'cpu',
    ...
    for type in model_types:
        losses = torch.cat(loss_all[type]).sqrt()
    ...
    naive_losses = torch.stack(naive_losses, dim=0).sqrt()
    mean_loss = naive_losses.mean(0)
    std_loss = naive_losses.std(0)
    line = ax.plot(range(1, timesteps), mean_loss, label=f'naive "persistence" model')
    ax.fill_between(range(1, timesteps), mean_loss - std_loss, mean_loss + std_loss, alpha=0.2,
                    color=line[0].get_color())
        ...
        for nidx, data in enumerate(dataloader):
            ...
            for midx, model in enumerate(models):
                ...
                if departure:
                    ...
                    pred[midx][nights[nidx][1:timesteps + 1]] = y
...
if args.action == 'test':
    ...
    model_types = ['linear']#, 'mlp']#'linear+sigmoid', 'mlp']#, 'standard_mlp']
    model_labels = ['G_linear']#, 'G_mlp'] #'G_linear+sigmoid', 'G_mlp']#, 'standard_mlp']
    cons = False
    rec = True
    emb = 0
